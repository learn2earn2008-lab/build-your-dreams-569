...
test("selecting a new lead mid-poll does NOT widen the banner scope", async ({
  page,
}) => {
  const [ada, bea, cyd] = THREE_LEADS;

  // Retry Ada + Bea; they stay "pending" until poll >= 5 (≈ 12.5s). Cyd is
  // never retried and NEVER settles (stays "failed" the whole time), so it
  // remains retryable/selectable — it is the lead we newly select mid-poll.
  await installMocks(page, THREE_LEADS, (lead, { retried, poll }) => {
    if (!retried) return "failed"; // Cyd: retryable, never settles
    return poll >= 5 ? "sent" : "pending"; // ada & bea
  });
  ...
  // Newly select Cyd mid-poll. The selection toolbar picks it up, but the
  // banner scope is the retried set (Ada + Bea) — it must NOT expand to wait
  // on Cyd.
  await page.getByLabel(`Select ${cyd.name}`).click();
  await expect(page.getByText("1 selected")).toBeVisible();
  await expect(banner).toBeVisible();

  // Cyd stays "failed" (never settles), yet the banner still clears once the
  // originally retried Ada + Bea settle — proving the newly selected lead was
  // never folded into the banner's settling scope.
  await expect(banner).toBeHidden({ timeout: 15_000 });

  // Cyd is still selected and retryable after the banner clears; its selection
  // state is independent of the (now-finished) settling window.
  await expect(page.getByText("1 selected")).toBeVisible();
});

test("toggling a checkbox off then back on mid-poll leaves the banner scope intact", async ({
  page,
}) => {
  const [ada, bea, cyd] = THREE_LEADS;

  // Retry Ada + Bea; they stay "pending" until poll >= 5 (≈ 12.5s). Cyd is
  // never retried and never settles (stays "failed"), so its row checkbox is
  // available throughout — it is the checkbox we toggle off and back on.
  await installMocks(page, THREE_LEADS, (lead, { retried, poll }) => {
    if (!retried) return "failed"; // Cyd: retryable, never settles
    return poll >= 5 ? "sent" : "pending"; // ada & bea
  });
  ...
  // Select Cyd mid-poll so there is a checked box to toggle.
  await page.getByLabel(`Select ${cyd.name}`).click();
  await expect(page.getByText("1 selected")).toBeVisible();
  await expect(banner).toBeVisible();

  // Toggle Cyd OFF, then back ON. The selection toolbar follows each toggle,
  // but the banner never reacts — its scope is fixed to the retried set.
  await page.getByLabel(`Select ${cyd.name}`).click();
  await expect(page.getByText("1 selected")).toBeHidden();
  await expect(banner).toBeVisible();
  ...
  // Despite the off/on cycle (and Cyd never settling), the banner clears only
  // once the originally retried Ada + Bea settle.
  await expect(banner).toBeHidden({ timeout: 15_000 });
});
...
test("deselecting all originally selected leads mid-poll keeps the banner scoped to them", async ({
  ...
  // Retry Ada + Bea; they stay "pending" until poll >= 5 (≈ 12.5s). Cyd is
  // un-selected and stays "failed" — it is available to keep the table busy but
  // is not in the retried set.
  await installMocks(page, THREE_LEADS, (lead, { retried, poll }) => {
    if (!retried) return "failed"; // Cyd: retryable, never settles
    return poll >= 5 ? "sent" : "pending"; // ada & bea
  });
  ...
  // Deselect the originally selected leads mid-poll. The selection toolbar
  // collapses to the empty set, but the banner is driven by the retried set,
  // not the current checkbox selection.
  ...
  // Wait a moment to prove the banner stays up even after the original
  // selection has been entirely cleared.
  await page.waitForTimeout(3_000);
  await expect(banner).toBeVisible();

  // The banner clears only when the originally retried Ada + Bea settle.
  await expect(banner).toBeHidden({ timeout: 15_000 });
});
